...
public class RandomForest {
    private int treeNum; // number of trees to create (pick an odd number to prevent ties)
    ...
    public void loadData(Scanner IrisScan) {
        ...
        Iris in;
        String q;
        // files are CSV files; the first line is the order of attributes
        // the dataset has no missing attributes
        s = IrisScan.nextLine();
    ...
    public void buildForest() {
        ...
        Iris[] dataR;
        int x; // number of attributes for a singular tree
        boolean[] selected = new boolean[a]; // array for all possible attributes
        int s; // a selected attribute

        for (int i = 0; i < treeNum; i++) { // create each tree
            for (int j = 0; j < a; j++) // set to no selected attributes for the tree to be created
                selected[j] = false;
            ...
            int k = 0;
            if (x == 4) { // if the tree would take all attributes
                k = 4;
                for (int m = 0; m < 4; m++)
                    selected[m] = true;
            }
            ...
            // create data subset from determined size(can include repeats of the same
            // instance)
            for (int d = 0; d < treeDataSize; d++)
                dataR[d] = dataSet[(r.nextInt(dataSet.length))]; // select a random iris in the data set
    ...
    public String test(Iris I) {
        ...
        if (class1 > class2 && class1 > class3)// return the majority answer
            return "setosa";
    ...
    public void readForest(Scanner read) {
        if (read.hasNextLine()) {
            String r = read.nextLine().trim();
            if (r.equalsIgnoreCase("Random forest for iris plant")) {
                // Read the number of trees
                if (read.hasNextInt()) {
                    treeNum = read.nextInt();
                    ...
                    for (int i = 0; i < treeNum; i++) {
                        ...
                        // Read the tree details
                        forest[i].readTree(read);
                    }
                } else {
                    System.err.println("Error reading forest file: Missing number of trees.");
                }
            } else {
                System.err.println("Error reading forest file: Invalid file format.");
            }
        }
    }
